...
from job_plugin import JobSearchPlugin
...
app = FastAPI(title="Job Application API with Semantic Kernel")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def startup_event():
    ...
    kernel.add_plugin(JobSearchPlugin(), plugin_name="JobToolbox")
    logger.info("✅ Local JobToolbox plugin registered.")
...
async def get_latest_jobs_endpoint():
    """
    Returns the 10 most recent job openings in JSON format.
    """
    try:
        # Access the get_latest_jobs tool directly from the kernel
        latest_jobs_tool = kernel.plugins["JobToolbox"]["get_latest_jobs"]
        latest_jobs_result = await kernel.invoke(latest_jobs_tool)
        
        # Return the tool's JSON output directly to the client
        return latest_jobs_result.value
    except Exception as e:
        logger.error(f"Error fetching latest jobs from new endpoint: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to retrieve latest jobs.")
